...
- `review-csharp`：C#、XAML 与关联配置的只读评审，区分功能/生命周期缺陷、设计/复制风险、主干交付违规与格式问题
...
- `format-csharp/references/代码规范.md` 是格式规范来源；修改时同步 `review-csharp/references/代码规范.md`，两份内容保持字节一致。评审技能自己的 `compliance-check.md` 只定义只读边界和工具用法，不复制修复流程
...
已有安装先检查差异并备份，按用户明确选择的技能子目录复制完整包，保留无关文件；校验源文件与安装文件哈希及引用资源。安装不需要提交仓库，不运行文档里不存在的打包模块。
